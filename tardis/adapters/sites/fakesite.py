...
class FakeSiteAdapter(SiteAdapter):
    def __init__(self, machine_type: str, site_name: str) -> None:
        ...
        key_translator = StaticMapping(remote_resource_uuid='remote_resource_uuid', resource_status='resource_status',
                                       created='created', updated='updated', resource_boot_time='resource_boot_time')
    ...
    async def deploy_resource(self, resource_attributes: AttributeDict) -> AttributeDict:
        ...
        now = datetime.now()
        response = AttributeDict(remote_resource_uuid=uuid4().hex,
