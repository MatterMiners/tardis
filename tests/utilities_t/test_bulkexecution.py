...
from tardis.utilities.bulkexecution import BulkExecution
...
class TestBulkExecution(TestCase):
    @staticmethod
    async def execute(execution: BulkExecution, count: int, delay=None):
        tasks = []
    ...
    def test_bulk_size(self):
        ...
        for size in (1, 10, 100, 1000, 2, 3, 5, 7, 97, 2129):
            with self.subTest(size=size):
                execution = BulkExecution(CallCounter(), size=size, delay=0.1)
    ...
    def test_bulk_delay(self):
        ...
        bulk_delay = 0.05
        execution = BulkExecution(CallCounter(), size=2 ** 32, delay=bulk_delay)
    ...
    def test_delay_tiny(self):
        ...
        execution = BulkExecution(CallCounter(), size=2 ** 32, delay=sys.float_info.min)
        result = run_async(self.execute, execution, count=2048)
        ...
        for wrong_size in (0, -1, 0.5, 2j, "15"):
            with self.subTest(size=wrong_size):
                with self.assertRaises(ValueError):
                    BulkExecution(CallCounter(), size=wrong_size, delay=1.0)
        for wrong_delay in (0, -5, 17j, "10"):
            with self.subTest(delay=wrong_delay):
                with self.assertRaises((ValueError, TypeError)):
                    BulkExecution(CallCounter(), size=100, delay=wrong_delay)
